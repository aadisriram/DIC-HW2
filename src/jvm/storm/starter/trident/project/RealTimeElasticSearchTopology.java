...
public class RealTimeElasticSearchTopology {
    ...
    public static StormTopology buildTopology(String[] args, Settings settings, LocalDRPC drpc)
		...
		**/
		...
		//NodeJS Spout : Doesn't quite work!!!
    ...
    public static void main(String[] args) throws Exception {
	   ...
	   ***/
	   ...
	   //The Common Terms query is a modern way to control stop words from costing the search. 
	   //The cut-off frequency controls the data that shows up. It is a 2 step query, where the
	   //first query gets all the queries with important words and then a second for the low frequency
	   //terms and calculates score only for the results from 1st query. 
       String query1 = QueryBuilders.commonTerms("text", "love").cutoffFrequency(0.001F).buildAsBytes().toUtf8();

       //A mult match query can be used to query across multiple fields, in this case text and user 
       //fields.
	   String query2 = QueryBuilders.multiMatchQuery("love", "text", "user").type(MatchQueryBuilder.Type.PHRASE_PREFIX).buildAsBytes().toUtf8();
	   
	   //A fuzzy query uses Levenshtein distance between the query term and the actual words
